...
def send_noti_function():
    print("hello")
    API_key_val = 'o.kXz7nZBqhMta6LwJiRTeUWqNYXXpzFOk'
    pb = Pushbullet(API_key_val)
    push = pb.push_note("Caution Alert","A PERSON HAS BEEN FOUND ENTERING YOUR PREMISES")
...
def insert_into_text(Unique_obj):
    ...
    if(Unique_obj == 'person' and sent_noti):
        sent_noti = False
        send_noti_function()
...
def cam_open(Obj_mainwindow,label1):
    ...
    classNames = []
    classFile = r'F:\GEN2programming\PYTHONPROG\face_recog_gui\object_detector\labels.names'

    with open(classFile, 'rt') as f1:
        classNames = f1.read().rstrip('\n').split('\n')

    configPath = r'F:\GEN2programming\PYTHONPROG\face_recog_gui\object_detector\ssd_mobilenet_v3_large_coco_2020_01_14.pbtxt'
    weightsPath = r'F:\GEN2programming\PYTHONPROG\face_recog_gui\object_detector\frozen_inference_graph.pb'
...
Obj_mainwindow.geometry("800x500")
...
label1_img= ImageTk.PhotoImage(file = r'F:\GEN2programming\PYTHONPROG\face_recog_gui\object_detector\images\bg.jpg')
label1['style'] = 'CustomLabelStyle.TLabel'
style.configure('CustomLabelStyle.TLabel',image = label1_img)

label2_img= ImageTk.PhotoImage(file = r'F:\GEN2programming\PYTHONPROG\face_recog_gui\object_detector\images\text.png')
